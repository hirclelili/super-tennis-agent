...
const hotReferencesPath = path.join(__dirname, "data", "hot-references.json");
const hotReferenceRankingsPath = path.join(__dirname, "data", "hot-reference-rankings.json");
...
// Topic directions use a compact agent prompt. The decision-chain framework is
// still passed as background lenses, but the model is not forced through
// insight -> angles -> topics multi-step generation.
  ...
  return [
    {
      ...
      content: [
        ...
        "- topic：想要选题方向 / 拍什么 / 内容角度 / 标题备选 / 一组选题。用户说『小红书图文选题』『短视频选题』『10 个选题』『每个给标题、切入角度、封面文案、正文主线』都必须是 topic，不是 content。",
        "- content：想把某个想法或某条已有选题做成具体内容（小红书图文 / 短视频脚本 / 朋友圈）。只有用户明确要『写成一篇正文』『生成完整脚本』『做成具体图文内容』『把第N条做成小红书』时才是 content。这时要解析出：contentIdea（要做成内容的一句话想法）、targetFormat（用户点名的形态，没点名留空）、libraryRef（用户若指『选题库里那条 / 第N条 / 关于XX的那条』，给出匹配关键词或序号，否则 null）。真正的内容会在主面板的内容工作台里生成。",
        ...
        "【选题优先】只要用户明确说『选题/题目/标题备选/内容方向』，即使同时出现『小红书图文/短视频/朋友圈』，也必须走 topic；不要因为出现平台或内容形态就跳 content。",
    ...
    ...historyMessages,
    {
      ...
      content: JSON.stringify({
        ...
        constraints: [
          ...
          "『小红书图文选题』里的图文只是平台/形态约束，intent 仍是 topic，targetFormat 只能在 content 时填写。",
...
function normalizeReferenceSourceType(value) {
  const text = String(value || "").trim().toLowerCase();
  if (text === "video" || text.includes("视频")) return "video";
  if (["image", "normal", "note"].includes(text) || text.includes("图文")) return "image";
  return "unknown";
}

function referenceTypeLabel(type) {
  const normalized = normalizeReferenceSourceType(type);
  if (normalized === "video") return "视频";
  if (normalized === "image") return "图文";
  return "未知";
}
...
function referenceMetricScore(metrics = {}) {
  const raw = (metrics.likes || 0)
    + (metrics.collects || 0) * 2
    + (metrics.comments || 0) * 3
    + (metrics.shares || 0) * 2;
  return Math.round(Math.log10(raw + 1) * 20);
}

function splitReferenceQuery(query = "") {
  ...
  const normalizedTerms = knownTerms.map((term) => term === "森蝶" ? "森碟" : term.toLowerCase());
  return [...new Set([...tokens, ...normalizedTerms, text].filter((item) => item.length >= 2))].slice(0, 16);
}

async function loadReferenceRankMap() {
  try {
    const raw = await readFile(hotReferenceRankingsPath, "utf8");
...
async function searchHotReferences({ query = "", sourceType = "all", limit = 8 } = {}) {
  let items = [];
  try {
    const raw = await readFile(hotReferencesPath, "utf8");
    const data = JSON.parse(raw);
  ...
  const rankMap = await loadReferenceRankMap();
  const tokens = splitReferenceQuery(query);
  const normalizedType = sourceType === "all" ? "all" : normalizeReferenceSourceType(sourceType);
  const matches = items
    .map((item) => {
      const ranking = rankMap.get(item.id) || {};
      const type = normalizeReferenceSourceType(item.sourceType || item.raw?.type);
      const text = referenceText(item);
      const tokenScore = tokens.reduce((score, token, index) => {
        if (!token) return score;
        if (text.includes(token)) return score + (index === 0 ? 34 : 18);
        return score;
      }, 0);
      const titleBoost = tokens.some((token) => String(item.title || "").toLowerCase().includes(token)) ? 18 : 0;
      const rankScore = ranking.score?.total || 0;
      const metricScore = referenceMetricScore(item.metrics || {});
      const totalScore = tokenScore + titleBoost + rankScore * 0.8 + metricScore;
      return {
        item,
        ranking,
        type,
    ...
    .filter((entry) => {
      if (normalizedType !== "all" && entry.type !== normalizedType) return false;
      if (tokens.length && entry.tokenScore <= 0) return false;
      return true;
    })
    .sort((a, b) => b.totalScore - a.totalScore)
    .slice(0, Math.max(1, Math.min(Number(limit) || 8, 12)))
    .map(({ item, ranking, type }) => ({
      id: item.id,
  ...
  return { query, total: matches.length, items: matches };
}

function referenceSearchFallbackReply(profile, query, result) {
  const venueName = profile.shortName || profile.name || "你的网球场";
  if (!result.items.length) {
    return `参考库里暂时没查到「${query}」相关内容。可以换成更宽的关键词，比如人物名、事件名、运动关键词分开查。`;
  }
  ...
  const lines = top.map((item, index) => {
    ...
    return `${index + 1}. ${item.typeLabel}「${item.title}」：赞 ${metrics.likes || 0} / 藏 ${metrics.collects || 0} / 评 ${metrics.comments || 0}`;
  }).join("\n");
  return [
    `我在参考库里找到 ${result.items.length} 条「${query}」相关内容，前几条以${Object.entries(typeCounts).map(([k, v]) => `${k}${v}条`).join("、")}为主。`,
    "",
    lines,
    "",
    `对 ${venueName} 来说，建议只借「热点切入口和讨论结构」，不要直接消费人物本身。可以把它转成家长会关心的命题：长期运动、自信表达、亲子支持、孩子成长状态。`,
  ].join("\n");
}

function referenceSearchMessages(profile, query, result) {
  return [
    {
      role: "system",
      content: [
        "你是少儿网球场的运营策略 Agent。用户显式要求你查询爆款/热点参考库，你需要基于传入的公开参考帖做分析。",
        "你的任务不是生成完整内容，而是帮助运营者判断这个热点/参考能不能借、怎么借、不要怎么借。",
        "必须结合球场档案；不要八卦化，不做隐私推测，不照搬标题和正文，不把公众人物当作商业背书。",
        "回答要像真实 GPT 顾问：先给结论，再给可借角度、风险、下一步可执行动作。",
        "如果用户问的是小红书/短视频热点，要优先分析停留点、家长共鸣点、可迁移结构。",
      ].join("\n"),
    },
    {
      role: "user",
      content: JSON.stringify({
        taskType: "reference_search_analysis",
        venueProfile: profileForPrompt(profile),
        query,
        ...
        requiredOutput: [
          ...
          "适合本球场借的角度",
          "不建议借的角度",
          "可以继续触发的动作：生成选题/生成小红书图文/生成短视频脚本/保存热点方向",
        ],
      }),
    },
  ];
...
async function buildReferenceSearchAnalysis(profile, payload = {}) {
  const query = String(payload.query || payload.message || "").trim();
  const sourceType = payload.sourceType || "all";
  const result = await searchHotReferences({ query, sourceType, limit: payload.limit || 8 });
  const settings = await loadAiSettings();
  const resolved = resolveAiProvider(settings);
  let reply = referenceSearchFallbackReply(profile, query, result);
  let aiMeta = buildLocalAiMeta(settings);

  if (resolved && result.items.length) {
    const { provider, config } = resolved;
    const providerLabel = providerDefaults[provider]?.label || provider;
    try {
      reply = await callAiText(provider, config, referenceSearchMessages(profile, query, result));
      aiMeta = { source: "ai", provider: providerLabel, model: config.model };
    } catch (error) {
      aiMeta = {
        source: "fallback",
        provider: providerLabel,
        model: config?.model || "",
        error: error.message || "参考库分析失败，已回退本地摘要",
      };
    }
  }

  return { reply, references: result.items, query, aiMeta };
}
...
function normalizeBrief(brief) {
  ...
  const referenceItems = Array.isArray(brief.referenceItems)
    ? brief.referenceItems.slice(0, 8).map((item) => ({
      title: String(item?.title || "").trim(),
  ...
  const topicMode = ["hotspot_reference", "one_shot"].includes(brief.topicMode) ? brief.topicMode : "";
  if (!theme && !mustCover.length && !String(brief.primaryGoal || "").trim() && !contentType && !topicMode && !referenceItems.length) return null;
  return {
    theme,
    topicMode,
    referenceItems,
...
function fallbackDirectionTitle(angle = {}, insight = {}, index = 0) {
  const chainId = String(angle.chainId || "");
  const contentType = String(angle.contentType || "");
  const campaignTheme = String(insight.campaignTheme || insight.weeklyFocus || "").trim();
  if (contentType && contentType !== "explainer") {
    const subject = String(angle.title || angle.parentQuestion || "真实场景").replace(/^家长想看看真实的/, "");
    return `真实看看${subject}，家长可以观察什么`;
  }
  const titlesByChain = {
    why_tennis: "孩子第一次接触球类运动，家长先别急着比较项目",
    vs_other_sports: "篮球、游泳、羽毛球都在看，网球该怎么放进备选",
    growth_value: "孩子适不适合网球，第一次体验时看这几个信号",
    trust_pro: "第一次体验网球课，家长别只看孩子打得好不好",
    next_step: "带孩子体验网球前，家长可以先准备这几个问题",
  };
  if (chainId === "campaign_focus") {
    const theme = (!campaignTheme || campaignTheme.length > 18 || /图文|短视频|内容|营销|家长价值|不太|小红书/.test(campaignTheme))
      ? "开业前了解网球场"
      : campaignTheme;
    const variants = [
      `${theme}，别只看活动优惠，先看孩子愿不愿意上场`,
      `${theme}，家长真正想知道的不是价格，是孩子适不适合`,
      `${theme}，可以先用一次体验观察孩子的状态`,
      `${theme}，不急着报名，先看球场和教练怎么带第一次`,
    ];
    return variants[index % variants.length];
  }
  if (titlesByChain[chainId]) return titlesByChain[chainId];
  const question = String(angle.parentQuestion || "").replace(/[？?]$/, "");
  if (question) return `${question}，家长可以先这样判断`;
  const defaults = [
    "给孩子选运动，不用一开始就找标准答案",
    "孩子第一次上场的状态，比项目名字更值得观察",
    "不想被兴趣班带着走，家长可以先看这几个细节",
  ];
...
function directionsFromAngles(profile, normalizedTask, insight, angles, want = 0) {
  ...
  const raw = angles.slice(0, take).map((angle, index) => ({
    ...
    title: fallbackDirectionTitle(angle, insight, index),
...
function hotspotFallbackDirections(profile, normalizedTask, brief = {}, want = 0) {
  const take = Number(want) > 0 ? Math.max(1, Math.min(12, Math.floor(Number(want)))) : 8;
  const refs = Array.isArray(brief.referenceItems) ? brief.referenceItems : [];
  const hasTranslate = /翻译|英文|表达/.test([brief.theme, ...(brief.mustCover || []), ...refs.map((r) => `${r.title || ""} ${r.content || ""}`)].join("\n"));
  const base = [
    {
      title: hasTranslate ? "森碟给田亮翻译出圈后，我更想聊她这些年一直打网球" : "这个热点真正能借的，是孩子长期运动后的状态",
      parentQuestion: "孩子长期坚持一项运动，会不会慢慢影响他的状态？",
      purpose: "用翻译视频做开场，但主体落到森碟多年打网球和长期运动给孩子带来的状态变化。",
      structure: [
        "封面文案：森碟出圈后，我更想聊她一直打网球这件事",
        "正文主线：先承接翻译视频热度；马上转到她这些年一直打网球；再说长期运动里的规则、反馈、坚持和临场感；结尾落到家长给孩子选运动时可以先看长期参与感。",
      ],
    },
    {
      title: "不是突然的，田亮女儿这些年一直打网球这件事，挺值得家长想一想",
      parentQuestion: "孩子的自信，是不是很多时候来自长期被支持的一件事？",
      purpose: "从长期打网球切到家庭培养观：别只看结果，要看孩子怎么在长期练习中慢慢建立状态。",
      structure: [
        "封面文案：不是突然的，长期运动真的会改变孩子的状态吗？",
        "正文主线：先说大家看到的是一个出圈瞬间；再提醒长期运动背后是很多次练习、失败和再来；然后讨论家长陪孩子坚持一件事的意义；结尾轻轻引到先体验、先观察。",
      ],
    },
    {
      title: "别只羡慕森碟会翻译，先看看长期打网球给了她什么",
      parentQuestion: "长期运动给孩子的，除了体能还可能是什么？",
      purpose: "从家长羡慕表达能力切入，但落点放在长期网球训练中的反馈、抗挫和临场状态。",
      structure: [
        "封面文案：别只羡慕会翻译，先看看她坚持的运动",
        "正文主线：先写家长看到翻译视频后的羡慕；再提醒森碟长期打网球这个背景；展开网球训练里的等待、判断、失误后再来；结尾问家长是否也想让孩子先拥有一个可坚持的运动场景。",
      ],
    },
    {
      title: "森碟的成长感，给家长一个提醒：网球不只是消耗体力",
      parentQuestion: "给孩子选运动，除了强身健体，还能观察什么？",
      purpose: "把热点从技能表现转成成长状态，强调网球运动中的规则感、抗挫和判断力。",
      structure: [
        "封面文案：网球不只是消耗体力，孩子的状态也会被看见",
        "正文主线：先从森碟的成长感切入；再说网球不是单纯出汗；展开规则感、等待、判断、失败后再来；最后落到家长第一次体验网球时可以观察这些细节。",
      ],
    },
    {
      title: "为什么很多家长看完森碟，会重新理解“长期运动”这件事？",
      parentQuestion: "孩子的能力是靠一门课速成，还是靠长期环境慢慢长出来？",
      purpose: "从热点引出培养观念：不制造焦虑，而是讨论长期运动环境的重要性。",
      structure: [
        "封面文案：看完森碟，我想到的是长期运动",
        "正文主线：先承认大家会被翻译视频吸引；再说真正值得讨论的是她长期打网球的背景；然后写运动状态不是一次性结果；结尾提醒家长先给孩子一个可持续尝试的网球场域。",
      ],
    },
    {
      title: "孩子要不要学网球？森碟这个热点之外，我更建议家长先看这件事",
      parentQuestion: "家长借热点想到网球时，第一步该看什么？",
      purpose: "防止变成同款营销，把热点落到更克制的判断：孩子是否愿意进入运动场景。",
      structure: [
        "封面文案：不是看别人学什么，而是看孩子适不适合开始",
        "正文主线：先说热点会让很多家长重新注意网球；再明确不是明星孩子学什么就跟什么；然后给出观察点：愿不愿意上场、能不能接受反馈、有没有一点成就感；结尾引导先体验。",
      ],
    },
    {
      title: "从“风一样的女子”到网球少女，家长最该看到的不是标签",
      parentQuestion: "孩子一路长大，什么东西比外界标签更重要？",
      purpose: "借公众成长记忆，不消费标签，讨论孩子长期拥有一个热爱的运动场域。",
      structure: [
        "封面文案：从小被看见的孩子，真正重要的不是标签",
        "正文主线：先轻轻带到公众成长记忆；再说不要把孩子固定在标签里；展开长期运动如何让孩子有自己的节奏和身份感；结尾回到家长如何支持孩子找到一件愿意坚持的事。",
      ],
    },
    {
      title: "如果孩子能长期坚持网球，家长会慢慢看到什么变化？",
      parentQuestion: "长期运动的价值，怎么不夸张地跟家长讲？",
      purpose: "把热点转成可收藏的家长观察帖，但不写成夸大功效，主体围绕网球长期参与。",
      structure: [
        "封面文案：长期坚持网球，家长可以观察这些变化",
        "正文主线：先用热点引入长期运动；再列网球场景里的观察维度：规则感、抗挫、专注、身体协调、临场判断；每一点都强调可能性而非承诺；结尾邀请家长留言孩子年龄和运动状态。",
      ],
    },
  ];
  const raw = Array.from({ length: take }, (_, index) => {
    const item = base[index % base.length];
    return {
      id: makeTopicId("hot", item.title),
      contentType: "explainer",
      chainId: "hotspot_reference",
      contentGoal: index % 3 === 0 ? "价值" : (index % 3 === 1 ? "信任" : "认知"),
      parentQuestion: item.parentQuestion,
      reason: "热点正在被家长讨论，适合借势转成长期运动与孩子成长的话题。",
      title: item.title,
      ...
      materials: ["参考库热点素材", "球场空镜", "教练观点", "少儿网球训练器材"],
      cta: "如果你也在给孩子选一项能长期坚持的运动，可以先带孩子体验一次再判断。",
      risk: "不写森碟同款，不暗示公众人物背书，不承诺训练效果。",
    };
  });
  return decorateDirectionList(raw, profile, normalizedTask);
}

function titleHasSportAnchor(direction = {}) {
  return /网球|运动|打球|训练|球场|长期坚持|长期运动|挥拍/.test(String(direction.title || ""));
}

function fillHotspotDirectionsWithAnchors(directions = [], profile, normalizedTask, brief, want = 0) {
  const take = Number(want) > 0 ? Math.max(1, Math.floor(Number(want))) : directions.length || 8;
  const anchored = directions.filter(titleHasSportAnchor);
  if (anchored.length >= Math.min(take, directions.length || take)) return anchored.slice(0, take);
  const seen = new Set(anchored.map((item) => item.title));
  const fallback = hotspotFallbackDirections(profile, normalizedTask, brief, take)
    .filter((item) => !seen.has(item.title));
  return [...anchored, ...fallback].slice(0, take);
}

function normalizeHotspotDirections(directions = []) {
  return directions.map((direction) => ({
    ...direction,
    chainId: "hotspot_reference",
    contentType: "explainer",
    goal: ["junior", "trust", "daily"].includes(direction.goal) ? direction.goal : "junior",
    platforms: Array.isArray(direction.platforms) && direction.platforms.length ? direction.platforms : ["xhs"],
    formats: Array.isArray(direction.formats) && direction.formats.length ? direction.formats : ["xhs_image"],
    sourceLabel: topicSourceLabels.ai,
  }));
}

function topicDirectionOneShotMessages(profile, task, insight, framework, mode, brief, stagePolicy = DEFAULT_STAGE_POLICY, lockContentType = "", want = 0) {
  const showcaseLabels = showcaseLabelsFor(stagePolicy);
  const lockLabel = lockContentType ? lockContentTypeLabel(lockContentType, stagePolicy) : "";
  const countText = want > 0 ? `${want} 条（少而精，不要凑数）` : "6-8 条";
  const decisionLenses = (framework.chains || [])
    .filter((chain) => chain.id !== "campaign_focus" || mode !== "balanced")
    .map((chain) => ({
      id: chain.id,
      label: chain.label,
      contentGoal: chain.contentGoal,
  ...
  const showcaseTypes = (stagePolicy?.availableShowcase || []).map((ct) => ({
    id: ct.id,
    label: ct.label,
    contentGoals: ct.primaryGoals,
    usableWhen: ct.compliance,
  }));
  ...
  return [
    {
      role: "system",
      content: [
        "你是一个懂少儿网球场运营的小红书/短视频选题合伙人。",
        "这不是填表任务。你的目标是一次性产出有洞察、有点击理由、能进入内容生产的原创选题。",
        "决策链、内容目标和真实展示类型只是背景镜头，用来提醒你别漏掉家长视角；不要机械覆盖、不要按字段逐条套模板。",
        "优先从家长真实决策瞬间出发：犹豫、比较、担心、观察、第一次体验后的判断，而不是从机构宣传出发。",
        "标题要像真实小红书用户愿意点开的表达，避免泛泛的「为什么」「几岁合适」「三点建议」流水线标题，除非你能写出新鲜角度。",
        "可以借鉴爆款的普适机制（降低决策成本、可收藏清单、共鸣型观察、反常识提醒），但不能复制任何外部原文或虚构案例。",
        "必须基于球场档案，不编造价格、开放时间、学员案例、爆满现场、升学或效果承诺。",
        "输出必须是严格 JSON，不要 Markdown，不要解释。",
      ].join("\n"),
    },
    {
      role: "user",
      content: JSON.stringify({
        taskType: "topic_directions_one_shot",
        venueProfile: profileForPrompt(profile),
        operatingInput: {
          mode: operatingModeLabels[task.mode] || task.mode || "日常运营",
          goal: goalLabels[task.goal] || task.goal || "日常运营",
        ...
        contextHints: {
          ...
          baselineInsight: {
            ...
            topQuestions: insight.topQuestions,
            parentSegments: insight.parentSegments,
            shootableAssets: insight.shootableAssets,
          },
          stageContext: {
            stage: stagePolicy?.label || stageLabels[profile.stage] || profile.stage,
            availableShowcase: showcaseLabels,
          ...
          contentRules: framework.contentRules || {},
          excludeTitles: (task.excludeTitles || []).slice(0, 40),
        },
        requiredShape: {
          workingContext: {
            weeklyFocus: "string，本次选题共同围绕的真实运营问题",
            parentQuestions: ["string，家长口语化问题，3-5 条"],
            usedLenses: ["string，只列本次真正用到的 2-4 个思考镜头"],
            qualityBar: ["string，本批选题的质量标准，2-4 条"],
          },
          directions: [{
            id: "string，ai_ 前缀",
            contentType: "explainer 或真实展示类 id",
            chainId: "string，来自 decisionLenses 或 showcaseTypes；只是归类，不要机械套",
            contentGoal: "认知|比较|价值|信任|行动|活动",
            parentQuestion: "string，家长会怎么问",
            reason: "string，为什么现在值得做，1 句",
            title: "string，像小红书/短视频真实可用选题",
            purpose: "string，家长在纠结什么 + 这条解决什么",
            goal: "opening|booking|junior|adult_beginner|community|event|trust|daily",
            audiences: ["parents"],
            platforms: ["xhs"],
            formats: ["xhs_image"],
            structure: ["string，后续生成内容可直接展开的顺序，3-5 步"],
            materials: ["string，真实可拍/可写素材"],
            cta: "string，轻转化，不硬卖课",
            risk: "string，不要写过头的边界",
          }],
        },
        constraints: [
          `输出 ${countText}。`,
          lockContentType ? `【硬锁】所有 directions.contentType 必须 = ${lockContentType}（${lockLabel}），禁止出现其它 contentType。` : "contentType 必须在本阶段可用类型内；开业前没有真实学员时，不要写学员成长/真实课堂。",
          "每条都必须面向 parents/teens；不要生成成人球友、白领自练、公司团建角度。",
          "platforms 只用 xhs、douyin、video、moments、group、dm；formats 只用 video、xhs_image、moments_text、moments_image、community。",
          "不要为了覆盖链路而产出同质题；宁可少，也要每条有不同用户问题、不同表达抓手。",
          "至少一半选题要带有明确的小红书停留理由：共鸣、反常识、可收藏、具体场景、家长决策瞬间。",
          "structure 要能服务后续内容生产，不能只写抽象概念。",
        ],
        outputNote: "只返回 JSON。",
      }),
    },
...
function isHotspotReferenceBrief(brief = {}, task = {}) {
  const text = [
    brief.topicMode,
    brief.theme,
    brief.toneOverride,
  ...
  return brief.topicMode === "hotspot_reference"
    || Array.isArray(brief.referenceItems) && brief.referenceItems.length > 0
    || /热点|爆款库|参考库|参考结构|借势|森碟|田亮|田雨橙/.test(text);
}

function hotspotTopicDirectionMessages(profile, task, brief, framework, want = 0) {
  const countText = want > 0 ? `${want} 条` : "8-10 条";
  return [
    {
      role: "system",
      content: [
        "你是一个很会做小红书热点借势的少儿网球场内容主编。",
        "这是「热点借势选题」任务，不是常规家长决策链任务。不要机械输出网球好处、几岁合适、三点建议等科普模板。",
        "你要从热点本身出发，但热点只能做开场钩子，主体必须落到网球或长期运动；不能把口语、英语、翻译能力当成主议题。",
        "正确路径是：翻译视频/森碟出圈 → 她长期打网球/长期运动的背景 → 家长能借此思考孩子运动培养。",
        "标题可以自然出现热点人物名或事件名，但不能八卦化、不能消费未成年人、不能暗示对方为本球场背书。",
        "目标是给广州家长看的少儿网球小红书图文选题：有点击理由、有观点、有共鸣、有正文展开空间。",
        "每条必须包含：标题、切入角度、封面文案、正文主线。字段映射为 title、purpose、structure。",
        "不要照搬参考帖标题或正文；只借热点切口、情绪机制和内容结构。",
        "遵守 contentRules：不承诺训练效果/升学，不编造案例、价格、开放时间。",
        "输出必须是严格 JSON，不要 Markdown，不要解释。",
      ].join("\n"),
    },
    {
      role: "user",
      content: JSON.stringify({
        taskType: "hotspot_reference_topic_directions",
        venueProfile: profileForPrompt(profile),
        hotspotBrief: {
          theme: brief.theme,
          mustCover: brief.mustCover || [],
          mustAvoid: brief.mustAvoid || [],
          toneOverride: brief.toneOverride || "",
          references: brief.referenceItems || [],
        },
        contentRules: framework.contentRules || {},
        requiredThinking: [
          "先判断热点的可借点，但只把翻译/口语作为引子，不作为内容主线。",
          "把热点转成网球场视角：长期打网球、长期运动、孩子状态、规则感、抗挫、临场判断、亲子支持。",
          "每条选题都要有不同的情绪切口，避免全都写成『长期运动带来自信』，但标题和封面必须有网球/运动锚点。",
          "标题要像小红书会点开的标题，不要像机构公众号。",
        ],
        requiredShape: {
          workingContext: {
            hotspotCore: "string，这个热点为什么被关注",
            usableAngles: ["string，适合本球场借的角度"],
            avoidAngles: ["string，不建议借的角度"],
            qualityBar: ["string，本批选题的质量标准"],
          },
          directions: [{
            id: "string，ai_ 前缀",
            contentType: "explainer",
            chainId: "hotspot_reference",
            contentGoal: "认知|比较|价值|信任|行动",
            parentQuestion: "string，家长看到这个热点后心里会冒出的真实问题",
            reason: "string，为什么现在值得发，1句",
            title: "string，选题标题；可自然带热点名，但标题必须能看出落点是网球/运动/长期训练",
            purpose: "string，切入角度：这条从热点切到哪个网球或长期运动相关的家长价值问题",
            goal: "junior|trust|daily",
            audiences: ["parents"],
            platforms: ["xhs"],
            formats: ["xhs_image"],
            structure: [
              "封面文案：string",
              "正文主线：string，用 3-5 个短句说明展开顺序"
            ],
            materials: ["string，真实可用素材，如热点截图只作灵感不直接搬运、球场空镜、教练观点等"],
            cta: "string，轻转化，不硬卖课",
            risk: "string，边界提醒",
          }],
        },
        constraints: [
          `输出 ${countText}。`,
          "所有 directions.chainId 必须是 hotspot_reference，contentType 必须是 explainer。",
          "所有标题或封面文案必须出现至少一个运动锚点：网球、运动、打球、训练、球场、长期运动、长期坚持、挥拍。",
          "标题中至少 4 条可以带『森碟』或『田亮』或『翻译视频』，但必须同时落到网球/运动，不能只聊口语、英语、翻译。",
          "至少 4 条标题要从家长共鸣出发，例如『我反而注意到…』『别只羡慕…』『不是突然…』这类真实表达，但不能脱离网球。",
          "每条 structure 第一项必须以『封面文案：』开头，第二项必须以『正文主线：』开头。",
          "不要写『森碟同款』『明星女儿都在学』『学网球就能…』这类低级或违规表达。",
          "不要生成以口语/英语/翻译能力为主体的选题；这类内容会偏离网球场运营目标。",
          "不要生成普通科普题：如『孩子几岁学网球合适』『网球的三大好处』，除非和热点有明确新角度。",
        ],
        outputNote: "只返回 JSON。",
      }),
    },
...
async function buildTopicDirectionsWithAi(profile, task = {}) {
  ...
  let directions = [];
  const hotspotMode = isHotspotReferenceBrief(brief || {}, task);
  ...
  if (resolved) {
    ...
    let stepFailed = null;
    try {
      const topicText = await callAiText(provider, config, hotspotMode
        ? hotspotTopicDirectionMessages(profile, task, brief || {}, framework, want)
        : topicDirectionOneShotMessages(profile, task, insight, framework, mode, brief, stagePolicy, lockContentType, want));
      ...
      const workingContext = topicData.workingContext && typeof topicData.workingContext === "object"
        ? topicData.workingContext
        : {};
      if (workingContext.weeklyFocus || Array.isArray(workingContext.parentQuestions)) {
        insight = {
          ...insight,
          weeklyFocus: String(workingContext.weeklyFocus || insight.weeklyFocus || "").trim(),
          topQuestions: Array.isArray(workingContext.parentQuestions) && workingContext.parentQuestions.length
            ? workingContext.parentQuestions.map(String).slice(0, 6)
            : insight.topQuestions,
          generationMode: mode,
          usedLenses: Array.isArray(workingContext.usedLenses) ? workingContext.usedLenses.map(String).slice(0, 4) : [],
          qualityBar: Array.isArray(workingContext.qualityBar) ? workingContext.qualityBar.map(String).slice(0, 4) : [],
        };
      }
      steps.push(hotspotMode ? "hotspot_reference" : "one_shot");
  ...
  if (!directions.length) {
    directions = hotspotMode
      ? hotspotFallbackDirections(profile, normalizedTask, brief || {}, want)
      : directionsFromAngles(profile, normalizedTask, insight, angles, want);
  }

  if (hotspotMode) {
    directions = normalizeHotspotDirections(directions);
    directions = fillHotspotDirectionsWithAnchors(directions, profile, normalizedTask, brief || {}, want);
  }

  // 硬锁兜底：无论 AI/critic 是否守规，最终只放该类型；若全被过滤掉则用本地锁定角度重建。
  if (lockContentType && !hotspotMode) {
  ...
  const summary = {
    ...
    suggestion: insight.weeklyFocus
      ? `本次围绕「${insight.weeklyFocus}」生成选题，挑选后保存入库。`
      : "已结合球场档案与运营目标生成选题方向，挑选后保存入库。",
...
function isHotspotTopicContext(context = {}) {
  const text = [
    context.chainId,
    context.topicMode,
    context.title,
    context.parentQuestion,
    context.purpose,
    context.topicAngle,
    ...(Array.isArray(context.keyPoints) ? context.keyPoints : []),
  ].filter(Boolean).join(" ");
  return context.chainId === "hotspot_reference" || /热点|森碟|田亮|翻译视频|参考库|借势/.test(text);
}

function videoDurationPolicy(contentType, context = {}) {
  const type = String(contentType || "").trim();
  if (isHotspotTopicContext(context)) {
    return {
      kind: "hotspot",
      durationHint: "30-40s",
      estimatedDurationSeconds: 36,
      shotRange: [4, 5],
      narrationCharRange: [110, 170],
      defaultSteps: ["热点一句带入", "球场观察", "动作细节", "家长判断", "轻引导"],
    };
  }
...
function removeDuplicateHookShot(script = [], hook = {}) {
  ...
  if (sameVideoLine(first.narration, hook.narration) || sameVideoLine(first.onScreenText, hook.onScreenText)) {
    return script.slice(1).map((shot, index) => ({ ...shot, id: index + 1 }));
  }
  return script;
}
...
function buildVideoMaterial(profile, topic) {
  ...
  const policy = videoDurationPolicy(topic.contentType, topic);
  const isHotspot = policy.kind === "hotspot";
  ...
  const hookNarration = isHotspot
    ? `森碟这次翻译视频很火，但我更想看她长期打网球留下的状态。`
    : isShowcase
    ? `先带你真实看看${title}。`
  ...
  const script = structure.map((step, index) => {
    const narration = isHotspot
      ? [
        "不是说学网球就会变成谁，而是长期在球场里，孩子会反复遇到接球、失误、等待和再来。",
        "这些时刻很小，但会让家长看到孩子面对变化时，是躲开，还是愿意再试一次。",
        "所以借这个热点，真正值得聊的不是明星故事，而是孩子能不能拥有一个长期参与的运动场景。",
        "如果你也在给孩子选运动，可以先带他上一次球场，看看他愿不愿意继续。",
      ][index % 4]
      : isShowcase
...
function buildContentBrief(profile, topic, task = {}) {
  ...
  const base = {
    ...
    eventInfo: task.eventInfo || "",
    source: topic.source || "",
    chainId: topic.chainId || task.topic?.chainId || "",
  };
  return base;
}
...
function formatSpecFor(format, brief = {}) {
  ...
  if (formatType === "video") {
    const policy = videoDurationPolicy(brief.contentType, brief);
    const isShowcase = policy.kind === "showcase";
    const isHotspot = policy.kind === "hotspot";
    spec.constraints.push(
      ...
      "script 不要重复 hook 的口播；第一条分镜要从 hook 之后继续推进。",
    ...
    if (isHotspot) {
      spec.constraints.push(
        "本选题是热点借势类：热点只占开头 1 句，不能把整条视频写成热点评论、英语口语、翻译能力或明星成长分析。",
        "主体必须回到网球场：孩子在接球、失误后再来、等待轮次、听教练反馈、调整站位等真实动作里呈现出的状态。",
        "不要使用硬套结构：不要按『定义概念→拆解三点→升华价值→CTA』写；要像一条真实短视频：一个观察 + 三个球场画面 + 一个家长判断。",
        "标题、封面、hook、publishCopy 都必须能看出落点是网球/长期运动，而不是口语/表达能力。",
        "script 控制 4-5 个镜头：热点带入、球场动作、失误/等待/调整细节、家长判断、轻引导。每个镜头只讲一个观察，不要长段说教。",
        "口播要像教练/运营者在球场边讲观察，短句、具体、少抽象词；少用『很多家长都会问』这类模板句。",
      );
    } else if (isShowcase) {
      spec.constraints.push(
...
function normalizeMaterial(raw, format, fallback) {
  ...
  if (formatType === "video") {
    ...
    const normalizedScriptRaw = Array.isArray(input.script) && input.script.length
      ...
      : fallback.script;
    const normalizedScript = removeDuplicateHookShot(normalizedScriptRaw, hook);
    const material = {
      type: "video",
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && pathname === "/api/references/search") {
      const body = await readJson(req);
      sendJson(res, 200, await searchHotReferences({
        query: body.query || body.message || "",
        sourceType: body.sourceType || "all",
        limit: body.limit || 8,
      }));
      return;
    }
    ...
    if (req.method === "POST" && pathname === "/api/agent/reference-search") {
      const body = await readJson(req);
      const profile = body.profile || await loadProfile().catch(() => ({}));
      sendJson(res, 200, await buildReferenceSearchAnalysis(profile, body));
      return;
    }
